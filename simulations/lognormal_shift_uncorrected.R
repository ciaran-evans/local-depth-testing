...
for(j in 1:length(ds)){
  ...
  for(i in 1:nsamp){
    ...
    samp1 = exp(rmvnorm(N1, mean = rep(0, d)))
    samp2 = exp(rmvnorm(N2, mean = rep(delta/sqrt(N1 + N2), d)))
...
for(j in 1:length(ds)){
  ...
  for(i in 1:nsamp){
    ...
    samp1 = exp(rmvnorm(N1, mean = rep(0, d)))
    samp2 = exp(rmvnorm(N2, mean = rep(delta/sqrt(N1 + N2), d)))
    samp3 = exp(rmvnorm(N2, mean = rep(0, d)))
...
write_csv(output, file = "lognormal_shift_results_uncorrected.csv")
