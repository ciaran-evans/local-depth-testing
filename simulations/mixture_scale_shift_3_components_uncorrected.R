...
for(j in 1:length(ds)){
  ...
  d <- ds[j]
  mu = matrix(c(rep(0, d), rep(2, d), rep(4, d)),
              nrow = 3, byrow=T)
  sigma = matrix(1, 3, d)
  sigma_new = matrix(1 + delta/sqrt(N1 + N2), 3, d)
  ...
  for(i in 1:nsamp){
    ...
    samp1 <- rmvnormmix(N1, lambda = alpha, mu = mu, sigma = sigma)
    samp2 <- rmvnormmix(N2, lambda = alpha, mu = mu, sigma = sigma_new)
    ...
    pvals_lcd[i] <- suppressMessages(depthTest(samp1, samp2, "lcd",
                                               loo_correction = F)$pval)
    pvals_pald[i] <- suppressMessages(depthTest(samp1, samp2, "pald",
                                                loo_correction = F)$pval)
...
for(j in 1:length(ds)){
  ...
  for(i in 1:nsamp){
    ...
    samp1 <- rmvnormmix(N1, lambda = alpha, mu = mu, sigma = sigma)
    samp2 <- rmvnormmix(N2, lambda = alpha, mu = mu, sigma = sigma_new)
    samp3 <- rmvnormmix(N2, lambda = alpha, mu = mu, sigma = sigma)
...
write_csv(output, file = "mixture_results_3_components_uncorrected.csv")