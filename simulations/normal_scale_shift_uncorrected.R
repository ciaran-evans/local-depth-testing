...
for(j in 1:length(ds)){
  ...
  for(i in 1:nsamp){
    ...
    samp1 = rmvnorm(N1, sigma=diag(d))
    samp2 = rmvnorm(N2, sigma=diag(rep((1 + delta/sqrt(N1 + N2))^2, d)))
...
write_csv(output, file = "normal_scale_shift_uncorrected_results.csv")
